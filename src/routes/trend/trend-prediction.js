...
export default class TrendPrediction extends React.PureComponent {
  ...
  onChange = (key) => {
    if (key === '1') { //全局的时候按照词频排序
      ...
      trendDataNew.terms.sort((a, b) => {
        return b.sum - a.sum;
      });
      if (trendDataNew.terms[0].t.toLowerCase() === this.props.query) {
        trendDataNew.terms = trendDataNew.terms.slice(1, trendDataNew.terms.length);//减去词频最高的词语
      }
      this.renderHist();
      this.renderTermTrend(trendDataNew.terms[0]);
    } else {
      d3.select('.active').classed('active', false);
      d3.select(this.parentNode).classed('active', 'true');
      d3.selectAll('.term').remove();
      ...
      const hist = timeline.append('g').selectAll('.term').data(trendDataNew.terms).enter()
        .append('g')
        .attr('class', 'term')
        .attr('id', (d) => {
  ...
  initChart = (term) => {
    ...
    /*
     ...
     return this.updateTrend(this.props.query);
     });
     d3.select('#trend').on('click', () => { // trend被点击的时候重新加载
     return this.renderTrend('big data', 1, 1000);
     });*/

    terms = {};
    maxSum = 0;
    // current hotsopt中按各技术2010年之后的文献总数排序
    trendDataNew.terms.forEach((t) => {
      t.sum = 0;
      t.year.forEach((tt) => {
        if ((tt.y > 2010) && (tt.d > 0)) {
          t.sum += tt.d;
    ...
    people = {};
    trendDataNew.authors.forEach((t) => {
      people[t.id] = t;
    });
  ...
  updateTrend = (query) => {
    ...
    if (this.isHotTerm(term) !== false) {
      const res = wget(`/lab/trend-prediction/${term}.json`);
      res.then((data) => {
        trendData = data;
        const dd = wget(`http://166.111.7.173:5012/trend/${term}`);
        dd.then((kk) => {
          trendDataNew = kk;
          console.log(trendDataNew);
          this.initChart(term);
        });
      });
    } else {
      const res = externalRequest(`http://166.111.7.173:9997/${term}.json`);
      res.then((data) => {
        trendData = data.data;
        this.initChart(term);
      });
  ...
  renderHist = () => {
    timeline = d3.select('#hist-chart').append('svg').attr('overflow', 'scroll').attr('z-index', 2).attr('id', 'wordid').attr('overflow-y', 'scroll');//里面的高度
    timeline.attr('height', () => {
      return (21 * trendDataNew.terms.length); //svg高度，每个词是20
    });
    ...
    const histGraph = timeline.append('g').selectAll('.term').data(trendDataNew.terms).enter()
      .append('g')
      .attr('class', 'term')
      .attr('transform', (d, i) => {
        termByFreq[i] = d;
  ...
  renderTermTrend = (data) => {
    ...
    termTrendStreamGraph = chart.append('g').attr('transform', () => {
    ...
    const termTrendEventNodes = termTrendStreamGraph.append('g').selectAll('.people').data(data.first.sort((a, b) => {
      return a.y - b.y;
    })).enter()
      .append('g')
      .attr('class', 'people')
  ...
  renderAxis = () => {
    timeSlidesDict = {};
    timeSlidesOffset = {};
    trendDataNew.time_slides.forEach((time, i) => {
      time.sort();
      time.forEach((year, j) => {
    ...
    timeWindow = trendDataNew.time_slides[0].length;
    x = (year) => {
      return ((timeSlidesDict[year] + ((1 / timeWindow) * timeSlidesOffset[year]))
        * (width / trendDataNew.time_slides.length));
    };
    axis = chart.append('g').selectAll('.axis').data(trendDataNew.time_slides).enter()
      .append('g')
      .attr('class', 'axis')
      .attr('transform', (d, i) => {
        return `translate(${((i * width) / trendDataNew.time_slides.length)},${0})`;// 需调整参数，点离左边空白处
      });

    const axisWidth = width / trendDataNew.time_slides.length;
  ...
  renderTrend = (q, start, end) => {
    ...
    const terms = {};
    let cnt = 0;
    const topTerms = Object.keys(trendDataNew.term_freq)
      .sort((a, b) => trendDataNew.term_freq[b] - trendDataNew.term_freq[a]);
    ...
    trendDataNew.term_freq_by_slide.forEach((termFreq, idx) => {
    ...
    // trendDataNew.terms.forEach((term) => {
    //   let prevNode = null;
    ...
    sankey.nodes(nodes).links(links)
      .nodeOffset(width / trendDataNew.time_slides.length)
      .layout(100);
    link = svg.append('g')
      .selectAll('.link')
      .data(links).enter()
    ...
    //     d.x = d.pos * (width / trendData.time_slides.length);
    ...
    // item = svg.append('g').selectAll('.item').data(trendDataNew.terms).enter()
    //   .append('g')
    //   .attr('class', 'item')
    //   .attr('transform', (d) => {
