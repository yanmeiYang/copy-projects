...
import RgSearchNameBox from './RgSearchNameBox';
...
let EgoHeight = document.body.scrollHeight - 180;
let EgoWidth = document.body.scrollWidth - (24 * 2);
let d3;
...
export default class RelationGraph extends React.PureComponent {
  constructor(props) {
    ...
    if (this.props.stylObj) {
      const { height } = this.props.stylObj;
      EgoHeight = height;
      EgoWidth -= 100;
    }
  }
  ...
  startup = (t) => {
    ...
    const orderdraw = (ds) => {
      ...
      svg.selectAll('line').data(_edges).filter((e, i) => {
        ...
        .style('stroke', 'black')
        .style('opacity', 0.8);
      svg.append('g').attr('class', 'ceter').enter().append('text')
        .attr('dx', (d) => {
          return -20;
        })
        .text('s');
          ...
          if (_lastNode !== null) {
            ...
            if (!isstraight(d.index, _lastNode)) {
              ...
              if (res !== null && res[1] === res[2]) {
                ...
                const compare1 = getPaths(d.index, _lastNode);
                const compare2 = getPaths(_lastNode, d.index);
    ...
    const _drawNetOnly = (b) => {
      ...
      link = svg.append('g').attr('class', 'links').selectAll('line').data(_edges)
        .enter()
        .append('line')
      ...
      node = svg.append('g').attr('class', 'nodes').selectAll('circle').data(_nodes)
        .enter()
        .append('circle')
        .attr('r', (d) => {
          if (destroyNodesIndex.includes(d.index)) {
      ...
      nodes_text = svg.selectAll('.nodetext').data(_nodes).enter().append('text')
        .attr('class', 'initialText')
      ...
      final_text = svg.selectAll('.nodetextstyle').data(_nodes).enter().append('text')
    ...
    this.drawNet = (type) => {
      ...
      return d3.json(`https://api.aminer.org/api/search/person/ego?query=${type}&sort=h_index%20&size=${snum}`, (error, graph) => {
        try {
          this.props.getRelationData(graph.nodes.slice(0, 10));
        } catch (e) {
          console.log('no getRelationData');
        }
        let a,
          i,
          k,
        ...
        simulation = d3.forceSimulation(_nodes).velocityDecay(0.3)
          ...
          .force('link', setlink)
          .force('gravity', d3.forceCollide(height / 100 + 10).strength(0.6))
          .alpha(0.2)
  ...
  render() {
    ...
    const headerShow = this.props.headerShow !== false;
    let showShadow = true;
    if (this.props.stylObj) {
      showShadow = this.props.stylObj.showShadow;
    }
    return (
      <div>
        {headerShow &&
          ...
          <div className={styles.relationHeader}>
            <div className={styles.statAndAction}>
              ...
              <div className={styles.action}>
                <label className={styles.marginRight}><FM id="com.relationGraph.header.action"
                                                          defaultMessage="相关操作：" /></label>
                ...
                <Button className={classnames({
                  ...
                  <FM id="com.relationGraph.header.action.pauseAdjustment"
                      defaultMessage="暂停调整" />
                </Button>
                ...
                <Button className={classnames(styles.two_paths, {
                  ...
                  <FM id="com.relationGraph.header.action.one-pointExtension"
                      defaultMessage="单点扩展" />
                </Button>
              </div>
            </div>
        ...
        <div className={showShadow ? styles.rgvisActive : styles.rgvisDefault}>
