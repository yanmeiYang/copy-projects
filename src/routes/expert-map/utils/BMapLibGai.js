    ...
    (function () {
      ...
      MarkerClusterer.prototype._addToClosestCluster = function (marker) {
        ...
        for (var i = 0, cluster; cluster = this._clusters[i]; i++) {
          ...
          if (center) {
            const d = this._map.getDistance(center, marker.getPosition());
            if (d < distance) {
              distance = d;
      ...
      MarkerClusterer.prototype._removeMarkersFromMap = function () {
        for (var i = 0, marker; marker = this._markers[i]; i++) {
          marker.isInCluster = false;
          const tmplabel = this._markers[i].getLabel();
      ...
      Cluster.prototype.updateClusterMarker = function () {
        ...
        this._clusterMarker.addEventListener('mouseenter', (event) => {
          ...
          for (var i = 0; i < that._markers.length; i++) {
            const userinfo = that._markers[i].getLabel().content;
            const userid = userinfo.substring(userinfo.length - 30, userinfo.length - 6);
            userids[i] = userid;
            if (ids.indexOf(userid) == -1) {
              ids += `${userid},`;
            }
          }
          const maindom = that._clusterMarker._domElement;
          const newarray = userids.sort();// 对ID进行排序
          let newids = '';
    ...
    (function () {
      ...
      (function () {
        ...
        baidu.string.toCamelCase = function (source) {
          ...
          return source.replace(/[-_][^-_]/g, (match) => {
      ...
      TextIconOverlay.prototype._updateCss = function () {
        const style = this.getStyleByText(this._text, this._styles);
      ...
      TextIconOverlay.prototype._updatePosition = function () {
        if (this._domElement && this._position) {
          ...
          const pixelPosition = this._map.pointToOverlayPixel(this._position);
      ...
      TextIconOverlay.prototype._bind = function () {
        ...
        function eventExtend(e, be) {
          ...
          if (e && be && x && y && elem) {
            const offset = T.dom.getPosition(map.getContainer());
        ...
        T.event.on(this._domElement, 'mouseenter', (e) => {
          me.dispatchEvent(eventExtend(e, new BaseEvent('onmouseenter')));
        });
        T.event.on(this._domElement, 'mouseout', (e) => {
          me.dispatchEvent(eventExtend(e, new BaseEvent('onmouseout')));
        });
        T.event.on(this._domElement, 'mouseleave', (e) => {
